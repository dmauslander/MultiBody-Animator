...
public class idx3d_Object extends idx3d_CoreObject
{
	...
	// D A T A  S T R U C T U R E S
		...
		public void rebuild()
		{
			...
			enumm=vertexData.elements();
			for (int i=vertices-1;i>=0;i--) vertex[i]=(idx3d_Vertex)enumm.nextElement();
			
			// Generate faster structure for triangles
			triangles=triangleData.size();
			triangle=new idx3d_Triangle[triangles];
			enumm=triangleData.elements();
			for (int i=triangles-1;i>=0;i--)
			{
				triangle[i]=(idx3d_Triangle)enumm.nextElement();
				triangle[i].id=i;
			}
		...
		public void removeDuplicateVertices()
		{
			...
			Enumeration enumm=edgesToCollapse.elements();
			while(enumm.hasMoreElements()) edgeCollapse((idx3d_Edge)enumm.nextElement());
